"""
MongoDB Resource Abstraction for UCore Framework.

This module defines the MongoDBResource class, a concrete implementation
of DatabaseResource for MongoDB, providing async connection management,
health checks, statistics, and collection operations.

Classes:
    MongoDBResource: MongoDB database resource for UCore.
"""
...
class MongoDBResource(DatabaseResource):
    """
    MongoDB database resource for UCore.

    Responsibilities:
        - Manage async MongoDB connection and database reference
        - Provide health checks and statistics
        - Execute queries and collection operations
    ...
    Args:
        ...
        max_idle_time: Max idle time for connections (seconds).
        health_check_collection: Collection name for health checks.

    Example:
        mongo = MongoDBResource("mongo", "mongodb://localhost:27017", "ucore_db")
        await mongo._connect()
        stats = await mongo.get_database_stats()
        await mongo._disconnect()
    """
    ...
    def _validate_connection_string(self, conn_string: str) -> None:
        ...
        if not conn_string.startswith("mongodb://") and not conn_string.startswith("mongodb+srv://"):
            ...
            raise ResourceError(
                ...
                code="MONGO_CONN_STRING_INVALID"
            )
