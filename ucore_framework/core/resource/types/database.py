"""
Database Resource Abstraction for UCore Framework.

This module defines the abstract DatabaseResource class, which provides
a unified interface for managing database connections, lifecycle, health
checks, and query execution for various database types (e.g., MongoDB, SQL).

Classes:
    DatabaseResource: Abstract base for all database resource implementations.
"""
...
class DatabaseResource(Resource, ABC):
    """
    Abstract database resource for UCore.

    Responsibilities:
        - Manage database connection lifecycle (connect/disconnect/cleanup)
        - Provide health checks and statistics
        - Execute queries and manage connection pools
    ...
    Args:
        ...
        health_check_query: Optional query for health checks.
