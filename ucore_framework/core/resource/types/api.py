"""
API Resource Abstraction for UCore Framework.

This module defines the APIResource class, which provides an async HTTP client
resource with connection management, retry logic, health monitoring, and
comprehensive error handling for RESTful APIs.

Classes:
    APIResource: Async HTTP client resource for API integration.
    ApiResource: Alias for compatibility.
"""
...
class APIResource(Resource):
    """
    Async HTTP API client resource for UCore.

    Responsibilities:
        - Manage HTTP connection pooling and session lifecycle
        - Provide async HTTP operations (GET, POST, PUT, PATCH, DELETE, HEAD)
        - Implement retry logic and exponential backoff
        - Monitor health and collect statistics
    ...
    Args:
        ...
        auth: Authentication information (bearer, basic, or API key).
        connection_pool_size: Max concurrent connections.
        max_keepalive_connections: Max keepalive connections per host.
        ssl_verify: Whether to verify SSL certificates.
        user_agent: User-Agent header value.

    Example:
        api = APIResource("my_api", "https://api.example.com")
        await api._connect()
        ...
        try:
            ...
            if parsed.scheme not in ['http', 'https']:
                ...
                raise ResourceError(
                    ...
                    code="API_URL_SCHEME_UNSUPPORTED"
                )
        except Exception as e:
            from ucore_framework.core.exceptions import ResourceError
            raise ResourceError(
                message=f"Invalid base URL '{self.base_url}': {e}",
